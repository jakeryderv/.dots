# The toolchain: every third-party CLI this machine needs, as a function of a
# nixpkgs package set. flake.nix builds it into the profile; a future NixOS
# or home-manager config can call the same function (`lib.tools` in the flake)
# and get the same list. Every non-obvious entry carries its reason inline.
pkgs:
with pkgs;
[
  ast-grep
